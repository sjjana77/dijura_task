...
export default function Expensedash({ month }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const data = await Apicall({ data: expensemode_filter, apiname: 'dashboard_data/income_category' });
            if (data) {
                setexpensemode([...data]);
            }
        };
    ...
    return (
        <div>
            ...
            <div style={{ padding: '10px', background: "#0d6efd", color: "white", marginBottom: "10px" }}>Income by Category</div>
